...
const UI_DICT = {
  // ヘッダ・概要
  "AshTale カード図鑑": "Tales of Wind Card Index",
...
function openModal(imageSrc) {
  ...
  // 画像の縦横比を CSS 変数 --modal-ar に渡す。CSS 側で
  // 「横いっぱい」か「縦いっぱい」の先に収まる方まで比率を保ったまま拡大する。
  const applyRatio = () => {
    if (!modalImg.naturalWidth || !modalImg.naturalHeight) return;
    modalImg.style.setProperty("--modal-ar", modalImg.naturalWidth / modalImg.naturalHeight);
  };

  modalImg.style.removeProperty("--modal-ar");
